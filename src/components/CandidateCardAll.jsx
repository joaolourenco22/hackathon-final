...
export default function CandidateCardAll({ candidate, index = 0, onClick, selected = false, onToggleSelected }) {
  const accentColors = [
    'var(--primary)',
    'var(--primary-2)',
    ...
    >
      ...
      {onToggleSelected && (
        <button
          ...
          className={`absolute top-3 left-3 w-6 h-6 rounded border flex items-center justify-center bg-white ${selected ? 'border-violet-600' : 'border-gray-300'} shadow`}
          title={selected ? 'Remover da comparação' : 'Selecionar para comparar'}
        >
          {selected && (
            <svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M16.704 5.29a1 1 0 0 1 .006 1.414l-7.25 7.333a1 1 0 0 1-1.436.012L3.29 9.39a1 1 0 1 1 1.42-1.408l3.01 3.039 6.53-6.6a1 1 0 0 1 1.454-.03Z" clipRule="evenodd"/></svg>
          )}
        </button>
      )}
      {/* Accent shape */}
      ...
      {candidate.soft_skills && (
        <div className="mt-4">
          <div className="text-xs text-gray-700 mb-1 text-center">Soft skills</div>
          <div className="flex justify-center">
            <div className="w-[260px]">
              {(() => {
                ...
                const axes = order;
                const values = order.map((k) => Number(candidate.soft_skills[k] ?? 0));
                return (
                  <Radar
                    width={260}
                    height={260}
                    axes={axes}
                    datasets={[{ values, color: '#6366f1' }]}
                    maxValue={5}
                    title={`Soft skills de ${candidate.name}`}
                  />
                );
              })()}
            </div>
